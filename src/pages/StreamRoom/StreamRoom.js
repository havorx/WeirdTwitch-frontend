...
export default function StreamRoom() {
    ...
    function getRoomDetail() {
        myAxios.get('/rooms/roomByName', {params: {roomName}}).then(async response => {
            if (response) {
    ...
    return (
        <article>
            <Container>
                <Row>
                    <Col xs={9}>
                        <StreamScreen member={audience.length} setTopic={setTopic}
                                      topic={topic} roomName={roomName}
                                      isStreamer={isStreamer}/>
                    </Col>
                    <Col className="room-border p-0" xs={3}>
                        <aside>
                            <StreamChat audience={audience} setTopic={setTopic} roomName={roomName}
                                        isStreamer={isStreamer}/>
                        </aside>
                    </Col>
                </Row>
